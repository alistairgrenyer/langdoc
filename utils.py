...
if __name__ == '__main__':
    ...
    # Create a dummy .gitdocrc for testing
    dummy_config_content = {
        "skip_dirs": [".git", "venv", "custom_skip"],
    ...
    print(f"Skip Dirs from config: {get_config_value(config, 'skip_dirs', [])}")
    print(f"LLM Model from config: {get_config_value(config, 'llm_model', 'gpt-3.5-turbo')}")
    print(f"Non-existent key: {get_config_value(config, 'non_existent', 'default_val')}")
    os.remove(CONFIG_FILE_NAME) # Clean up

    print("\n--- Testing Project Name --- ")
    print(f"Project name for '.': {get_project_name('.')}")
    print(f"Project name for '../': {get_project_name('..')}")

    print("\n--- Testing File Tree --- ")
    # Create some dummy files and dirs for tree testing
    if not os.path.exists('temp_tree_dir'): os.makedirs('temp_tree_dir/subdir1')
    if not os.path.exists('temp_tree_dir/subdir2'): os.makedirs('temp_tree_dir/subdir2')
    if not os.path.exists('temp_tree_dir/__pycache__'): os.makedirs('temp_tree_dir/__pycache__') # skipped
    open('temp_tree_dir/file1.py', 'w').close()
    open('temp_tree_dir/file2.txt', 'w').close()
    open('temp_tree_dir/subdir1/sfile1.py', 'w').close()
    open('temp_tree_dir/subdir2/sfile2.js', 'w').close()

    print("Full tree (max_depth 3):")
    print(get_file_tree('temp_tree_dir'))

    print("\nPython files tree (max_depth 2):")
    print(get_file_tree('temp_tree_dir', file_ext_filter='.py', max_depth=2))
    
    # Cleanup dummy tree dir
    import shutil
